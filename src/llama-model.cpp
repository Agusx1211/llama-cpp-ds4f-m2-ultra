        ...
        case LLM_ARCH_DEEPSEEK4:
            {
                ...
                if (hparams.n_layer_nextn > 0) {
                    if (params.ctx_type == LLAMA_CONTEXT_TYPE_MTP) {
                        filter = [&](int32_t il) { return il >= (int32_t) hparams.n_layer(); };
                    } else {
                        filter = [&](int32_t il) { return il <  (int32_t) hparams.n_layer(); };
                    }
                }
                ...
                if (params.ctx_type == LLAMA_CONTEXT_TYPE_MTP && hparams.n_dspark_block_size > 0) {
                    // Official DSV4 DSpark stages use a K-only latent SWA
                    // cache, unlike the target trunk and native MTP head.
                    ...
                    // The fork's native trunk and MTP graphs share the DSV4
                    // compressed cache hierarchy. Do not route MTP through the
                    // generic hybrid ISWA cache used by upstream's dense head.
